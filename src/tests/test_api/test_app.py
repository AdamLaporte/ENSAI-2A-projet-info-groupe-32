...
def auth_headers_user1(client):
    ...
    response = client.post(
        "/login", data={"username": "test_u1", "password": "test_u1"}
    )
...
def auth_headers_user2(client):
    ...
    """
    response = client.post(
        "/login", data={"username": "test_u2", "password": "test_u2"}
    )
    assert response.status_code == 200, "Échec de la connexion pour le setup de test"
    token = response.json()["access_token"]
    return {"Authorization": f"Bearer {token}"}
...
def test_register_ok(client):
    ...
    response = client.post(
        "/register", json={"nom_user": "new_user_api", "mdp": "password123"}
    )
...
def test_register_user_exists(client):
    ...
    response = client.post(
        "/register",
        json={"nom_user": "test_u1", "mdp": "password123"},
    )
...
def test_login_fail_password(client):
    ...
    response = client.post(
        "/login", data={"username": "test_u1", "password": "wrong_password"}
    )
...
def test_login_fail_user(client):
    ...
    response = client.post(
        "/login", data={"username": "unknown_user", "password": "password"}
    )
    assert response.status_code == 401
...
def test_create_qrcode_ok(client, auth_headers_user1):
    ...
    assert data["id_proprietaire"] == "1"
    assert "scan_url" in data
...
def test_get_stats_ok(client, auth_headers_user1):
    ...
    assert data["scans_recents"][0]["geo_city"] == "Mountain View"
