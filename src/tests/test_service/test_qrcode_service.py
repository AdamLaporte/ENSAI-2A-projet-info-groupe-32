...
def test_creer_qrc_suivi_ok():
    ...
    with (
        patch(
            "service.qrcode_service.generate_and_save_qr_png", return_value="/tmp/f.png"
        ) as gen_mock,
        patch(
            "service.qrcode_service.filepath_to_public_url",
            return_value="http://x/f.png",
        ),
    ):
        service = QRCodeService(fake_dao)
        with patch("service.qrcode_service.SCAN_BASE", "http://scan.me"):
            res = service.creer_qrc(
                url="https://ex.com", id_proprietaire="3", type_qrcode=True
            )
...
def test_creer_qrc_classique_ok():
    ...
    with patch(
        "service.qrcode_service.generate_and_save_qr_png", return_value="/tmp/x.png"
    ):
        with patch(
            "service.qrcode_service.filepath_to_public_url",
            return_value="http://x/x.png",
        ):
            service = QRCodeService(fake_dao)
...
def test_modifier_qrc_success():
    ...
    with (
        patch("service.qrcode_service.SCAN_BASE", "http://scan.me"),
        patch(
            "service.qrcode_service.generate_and_save_qr_png", return_value="/tmp/f.png"
        ),
        patch(
...
def test_modifier_qrc_statique_change_url_regenere(mock_gen_png):
    ...
    mock_gen_png.assert_called_once()
...
def test_modifier_qrc_dynamique_change_url_ne_regenere_pas(mock_gen_png):
    ...
    mock_gen_png.assert_not_called()
