...
def test_enregistrer_vue():
    ...
    """
    with patch.object(
        StatistiqueDao, "incrementer_vue_jour", return_value=True
    ) as mock_increment:
...
def test_get_statistiques_qr_code_detail_complet():
    ...
    mock_agregats = {
        "total_vues": 10,
        "premiere_vue": date(2025, 1, 1),
        "derniere_vue": date(2025, 1, 5),
    }
    ...
    with (
        patch(
            "service.statistique_service.StatistiqueDao.get_agregats",
            return_value=mock_agregats,
        ) as mock_get_agg,
        patch(
            "service.statistique_service.StatistiqueDao.get_stats_par_jour",
            return_value=mock_par_jour,
        ) as mock_get_jour,
        patch(
            "service.statistique_service.LogScanDao.get_scans_recents",
            return_value=mock_scans_recents,
        ) as mock_get_logs,
    ):
        service = StatistiqueService()
        ...
        assert resultat["premiere_vue"] == "2025-01-01"
