...
class TokenDao(metaclass=Singleton):
        ...
        try:
            with DBConnection().connection as connection:
                with connection.cursor() as cursor:
                    ...
                    if created:
                        logging.info(
                            f"Token créé avec succès pour user {token.id_user}"
                        )
                    else:
                        logging.error(
                            "Échec de la création du token : aucune ligne insérée."
                        )
    ...
    def existe_token(self, jeton: str) -> bool:
        ...
        try:
            ...
            logging.info(
                f"Erreur lors de la vérification de l'existence du token : {e}"
            )
            return False
